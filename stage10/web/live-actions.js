(() => {
  ...
  const checked=(root)=>[...root.querySelectorAll('input[type="checkbox"]:checked')].map(x=>x.value).filter(Boolean);
  let session=null;
  ...
  async function bindMessages(){
    ...
    const selectAll=view.querySelector('[data-message-select-all]');const clear=view.querySelector('[data-message-clear]');
  ...
  async function bindCorrections(){
    ...
    rows.forEach((row,i)=>{const item=items[i];if(!item)return;const accept=row.querySelector('.accept');const reject=row.querySelector('.reject');
      if(accept){accept.disabled=false;accept.textContent='Akceptuj';accept.addEventListener('click',e=>{e.preventDefault();busy(accept,async()=>{const expected=Number(item.expected_version);if(!Number.isInteger(expected)||expected<0)throw new Error('Korekta nie zawiera poprawnej expected_version.');await api.write('mol-app-v2-attendance-correct',{request_id:item.request_id||item.correction_id||api.requestId(),employee_id:item.employee_id,work_date:item.work_date,expected_version:expected,...(item.start_at?{start_at:item.start_at}:{}),...(item.stop_at?{stop_at:item.stop_at}:{}),reason:item.reason});setStatus('Korekta zaakceptowana w backendzie V2.','ok');location.reload();}).catch(err=>{setStatus(err.message,'error');accept.disabled=false;});},true);}
