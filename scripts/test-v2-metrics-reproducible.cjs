...
const es=JSON.parse(before.get('es-report-read.json')),normalize=es.nodes.find(n=>n.name==='Normalize Operators'),ret=es.nodes.find(n=>n.name==='Return');assert.ok(normalize&&ret);assert.ok(normalize.parameters.jsCode.includes("error_code:'ES_REPORT_EMPTY'"),'Empty ES report must be source-unavailable, not operator-not-found');assert.ok(ret.parameters.jsCode.includes("error_code:x.ok===true?null:x.error_code"),'ES reader must preserve the source error code');
const manifest=JSON.parse(fs.readFileSync(path.join(__dirname,'../backend/v2/manifest.json'),'utf8'));assert.equal(manifest.active_versions.es_report_read,'c90f9be9-c265-4ec0-90dc-13327124f607');assert.equal(manifest.active_versions.metrics_task_ack,'e688b2fd-af46-4a89-92c2-6e7aef5d22d0');
