...
test('plan uses default all-items mode and standard n8n item array',()=>{const p=node('Plan Persistence');assert.equal(Object.hasOwn(p.parameters,'mode'),false);assert.ok(p.parameters.jsCode.includes('const i=$input.first().json;'));assert.ok(p.parameters.jsCode.includes('return [{json:{execute:true,batch:plan.batch}}]'));assert.ok(p.parameters.jsCode.includes('return [{json:{execute:false'));});
test('plan avoids n8n validator false positive helper pattern',()=>{const c=node('Plan Persistence').parameters.jsCode;assert.ok(c.includes("const fields=['episode_id','employee_id','type','anchor','opened_at'];return fields.every"));assert.equal(c.includes("return ['episode_id','employee_id','type','anchor','opened_at'].every"),false);});
...
test('manifest records fresh controlled no-write runtime evidence',()=>{assert.equal(manifest.communication.es_alert_persistence_runtime_test_execution,'644321');assert.equal(manifest.communication.es_alert_persistence_runtime_subexecution,'644322');});
