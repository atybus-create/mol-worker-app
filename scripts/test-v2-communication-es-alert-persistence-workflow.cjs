...
test('plan uses per-item mode and a single n8n item output',()=>{const p=node('Plan Persistence');assert.equal(p.parameters.mode,'runOnceForEachItem');assert.ok(p.parameters.jsCode.includes('const i=$json;'));assert.ok(p.parameters.jsCode.includes('return {json:{execute:true,batch:plan.batch}}'));assert.equal(p.parameters.jsCode.includes('return [{json:{execute:true'),false);});
