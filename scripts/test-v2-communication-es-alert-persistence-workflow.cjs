...
test('plan uses all-items mode and standard n8n item array',()=>{const p=node('Plan Persistence');assert.equal(p.parameters.mode,'runOnceForAllItems');assert.ok(p.parameters.jsCode.includes('const i=$input.first().json;'));assert.ok(p.parameters.jsCode.includes('return [{json:{execute:true,batch:plan.batch}}]'));assert.ok(p.parameters.jsCode.includes('return [{json:{execute:false'));});
