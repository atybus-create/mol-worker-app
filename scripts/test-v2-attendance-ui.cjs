...
(async()=>{
  ...
  b.modes.offline=true;b.el('workStart').click();b.el('workStart').click();await settle();assert.equal(b.calls.filter(c=>c.body).length,1);assert.equal(b.el('attendanceRetry').hidden,false);assert.equal(b.el('workStart').disabled,false);
  ...
  const reloaded=browser(b.storage);reloaded.window.molAttendance.activate(profile,token);await settle();assert.equal(reloaded.el('attendanceRetry').hidden,false);assert.equal(reloaded.el('workStart').disabled,false);
  reloaded.el('workStart').click();await settle();assert.equal(reloaded.calls.find(c=>c.body).body.request_id,first);assert.equal(reloaded.storage.size,0);
  ...
  assert.equal(process.calls.filter(c=>c.body).length,7);assert.equal(process.timers.length,0);assert.equal(process.el('attendanceRetry').hidden,false);assert.equal(process.el('processStart').disabled,false);assert.ok(process.storage.size);
  ...
  console.log('Attendance UI PASS: confirmed state, double-click, same-button offline/reload recovery, process route/version, bounded busy retry with same request ID, cancellation after logout.');
