...
assert.equal(c.status,'REAL_ES_CONTEXT_VERIFIED_FAIL_CLOSED');
for(const [k,dk] of Object.entries(map)){
  ...
  assert.equal(d.active_versions[dk],x.active_version,dk+' active version drift');
}
...
assert.equal(c.safety.es_verified_live,false);
...
assert.equal(c.safety.real_positive_es_test_complete,true);
assert.equal(c.safety.real_source_context_verified,true);
...
assert.equal(c.real_data_evidence.match_process.classification,'MATCH_PROCESS');
assert.equal(c.real_data_evidence.match_process.wrong_process_delta,null);
assert.equal(c.real_data_evidence.match_process.work_outside_app_delta,null);
assert.equal(c.real_data_evidence.match_process.no_activity_verified,true);
assert.equal(c.real_data_evidence.match_process.no_activity_coverage_ready,false);
assert.equal(c.real_data_evidence.match_process.communication_write,false);
