(() => {
  ...
  const bootstrap=async()=>{const css=document.createElement('link');css.rel='stylesheet';css.href=versioned('./details.css');document.head.append(css);await load('../shared/states.js');await load('./details.js');await load('./worktime.js');await load('../shared/api.js');await load('../shared/regression-guards.js');await load('./live.js');await load('./live-actions.js');await load('./stage5.js');await load('./corrections.js');};
