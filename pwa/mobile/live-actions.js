(() => {
  ...
  async function bindWorkerMessages(){
    ...
    const data=await api.read('mol-app-v2-messages',{limit:25});const items=Array.isArray(data?.items)?data.items:[];let selected=null;let shownPromise=Promise.resolve();
    ...
    panel.addEventListener('click',e=>{const card=e.target.closest('[data-live-message]');if(!card)return;selected=items.find(x=>x.message_id===card.dataset.liveMessage)||null;if(!selected)return;syncAck();if(selected.shown_at){card.classList.add('is-read');return;}const current=selected;shownPromise=api.write('mol-app-v2-message-shown',{request_id:api.requestId(),message_id:current.message_id}).then(result=>{current.shown_at=result?.shown_at||new Date().toISOString();card.classList.add('is-read');if(selected===current)syncAck();}).catch(err=>{if(selected===current)syncAck();setStatus(err.message,'error');throw err;});},true);
    ack?.addEventListener('click',e=>{e.preventDefault();if(!selected)return;const current=selected;lock(ack,async()=>{await shownPromise;await api.write('mol-app-v2-message-ack',{request_id:api.requestId(),message_id:current.message_id});current.ack_at=new Date().toISOString();syncAck();setStatus('Odbiór komunikatu potwierdzony.','ok');}).catch(err=>{setStatus(err.message,'error');syncAck();});},true);
  }
