(() => {
  ...
  function applyState(state){
    ...
    if(start)start.disabled=attendance!=='NOT_STARTED';if(stop)stop.disabled=attendance!=='OPEN';document.querySelectorAll('.process-option').forEach(b=>{b.disabled=attendance!=='OPEN';});if(endProcess)endProcess.disabled=attendance!=='OPEN'||!active;if(logoutProcess)logoutProcess.disabled=attendance!=='OPEN'||!active;if(reopen){reopen.hidden=attendance!=='CLOSED';reopen.disabled=attendance!=='CLOSED';}
  ...
  function bindAttendanceAndProcess(){
    ...
    reopen?.addEventListener('click',e=>{e.preventDefault();lock(reopen,async()=>{const {body}=await common();await api.write('mol-app-v2-attendance-reopen',body);setStatus('Dzień przywrócony do OPEN.','ok');reload();}).catch(err=>{setStatus(err.message,'error');getState().then(applyState).catch(()=>{});});},true);
