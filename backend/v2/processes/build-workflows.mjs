...
// Invalid permission rows fail closed for new starts/changes, without blocking
// status, logout, STOP or recovery of an already authorized frozen command.
const permissionConfig="const config={};for(const c of $('Read Config').all().map(i=>i.json)){if(!c.key)continue;if(c.key.startsWith('ALLOWED_PROCESSES_')){if(Object.hasOwn(config,c.key)){config[c.key]=null;continue;}try{config[c.key]=JSON.parse(c.value_json);}catch{config[c.key]=null;}}else config[c.key]=JSON.parse(c.value_json);}const employeePermissionKey='ALLOWED_PROCESSES_EMPLOYEE_'+employee.employee_id,rolePermissionKey='ALLOWED_PROCESSES_ROLE_'+employee.role,permissionEmployee={...employee,allowed_processes:Object.hasOwn(config,employeePermissionKey)?config[employeePermissionKey]:config[rolePermissionKey]};";
replace('Decide',"const config={};for(const c of $('Read Config').all().map(i=>i.json))if(c.key)config[c.key]=JSON.parse(c.value_json);",permissionConfig);
replace('Decide',"snapshot_version:selected?.version||0,", "snapshot_version:selected?.version||0,...processSnapshot({attendance:selected,processes,now:new Date().toISOString()}),process_catalog:visibleProcessCatalog($('Read Catalog').all().map(x=>x.json),permissionEmployee).map(p=>({process_code:p.process_code,display_name:p.display_name})),process_permission_error:parseAllowedProcesses(permissionEmployee.allowed_processes).valid?null:'PROCESS_PERMISSION_CONFIG_INVALID',");
...
replace('Decide',"actor:auth.actor,employee,request_id:i.request_id,operation:i.operation", "actor:auth.actor,employee:permissionEmployee,request_id:i.request_id,operation:i.operation");
...
console.log(JSON.stringify(process.argv[2]?Object.fromEntries(['START','CHANGE','LOGOUT'].map(op=>[op.toLowerCase(),api(op,process.argv[2])])):g,null,2));
