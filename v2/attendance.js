(() => {
  ...
  let token='',employee='',role='',generation=0,readSequence=0,busy=false,snapshot=null,pending=null,sheetProposal=null,confirmedAt=0;
  ...
  function buttons(){for(const id of ['workDate','attendanceRefresh','workStart','workFinish','workReopen','correctionStart','correctionStop','correctionReason','correctionSave','sheetCorrectionId','sheetPreviewButton','sheetApproveButton','processChoice','processStart','processChange','processLogout'])el(id).disabled=busy||!!pending;
  ...
  function render(data){if(data.work_date!==el('workDate').value||data.employee?.employee_id!==employee)return;
    ...
    snapshot=data;confirmedAt=Date.now();const a=data.attendance;
  ...
  function processCommand(op){if(busy||pending||snapshot?.attendance?.state!=='OPEN')return;submit('process-'+op,{request_id:crypto.randomUUID(),work_date:snapshot.work_date,expected_version:snapshot.snapshot_version,...(op==='logout'?{}:{process_code:el('processChoice').value})});}
